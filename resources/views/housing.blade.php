...
@section('content')
    <div class="bg-theme-light-blue py-10 lg:py-14">
        <h1 class="section-title text-center">Hébergement</h1>
    </div>
    <div class="lg:max-w-(--breakpoint-xs) section-spacing container mx-auto px-4">
        <p class="text-center text-lg font-medium text-gray-600">Vous recherchez un hébergement pour le weekend de la FVSP 2026 ? Laissez-nous votre demande.</p>
        ...
        <form action="{{ route('housing.store') }}" method="POST" class="mt-10 grid gap-y-5">
            @csrf
            ...
            <div>
                <label for="message" class="block font-semibold text-gray-600">Votre message (optionel)</label>
                <div class="mt-2">
                    <textarea name="message" id="message" rows="5" class="focus:outline-theme-blue block w-full rounded-[8px] bg-gray-50 px-3 py-3 font-medium text-gray-800 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:bg-white focus:outline-2 focus:-outline-offset-2 sm:text-sm/6"></textarea>
                </div>
